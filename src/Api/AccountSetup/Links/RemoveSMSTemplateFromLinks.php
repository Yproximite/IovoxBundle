...
class RemoveSMSTemplateFromLinks extends AbstractLinks
{
    ...
    public function executeQuery(RemoveSMSTemplateFromLinksPayload $payload): bool
    {
        ...
        $validate = $this->validator->validate($payload, null, [RemoveSMSTemplateFromLinksPayload::GROUP_REMOVE]);
        if ($validate->count() > 0) {
            throw new ValidationPayloadException($validate);
        }

        $query->setContent($this->serializer->serialize($payload, 'xml', ['groups' => [RemoveSMSTemplateFromLinksPayload::GROUP_REMOVE]]));
    ...
    protected function setQueryParameters(): void
    {
        ...
        $this->allQueryParameters = array_merge([
            ...
            MethodQueryParameter::getParameterName()  => new MethodQueryParameter('removeSMSTemplateFromLinks'),
        ], $this->editableQueryParameters);
    }
