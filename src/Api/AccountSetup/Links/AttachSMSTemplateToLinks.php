...
class AttachSMSTemplateToLinks extends AbstractLinks
{
    ...
    public function executeQuery(AttachSMSTemplateToLinksPayload $payload): bool
    {
        ...
        $validate = $this->validator->validate($payload, null, [AttachSMSTemplateToLinksPayload::GROUP_ATTACH]);
        if ($validate->count() > 0) {
            throw new ValidationPayloadException($validate);
        }

        $query->setContent($this->serializer->serialize($payload, 'xml', ['groups' => [AttachSMSTemplateToLinksPayload::GROUP_ATTACH]]));
    ...
    protected function setQueryParameters(): void
    {
        ...
        $this->allQueryParameters = array_merge([
            ...
            MethodQueryParameter::getParameterName()  => new MethodQueryParameter('attachSMSTemplateToLinks'),
        ], $this->editableQueryParameters);
    }
