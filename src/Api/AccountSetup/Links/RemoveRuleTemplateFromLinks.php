...
class RemoveRuleTemplateFromLinks extends AbstractLinks
{
    ...
    public function executeQuery(RemoveRuleTemplateFromLinksPayload $payload): bool
    {
        ...
        $validate = $this->validator->validate($payload, null, [RemoveRuleTemplateFromLinksPayload::GROUP_REMOVE]);
        if ($validate->count() > 0) {
            throw new ValidationPayloadException($validate);
        }

        $query->setContent($this->serializer->serialize($payload, 'xml', ['groups' => [RemoveRuleTemplateFromLinksPayload::GROUP_REMOVE]]));
    ...
    protected function setQueryParameters(): void
    {
        ...
        $this->allQueryParameters = array_merge([
            ...
            MethodQueryParameter::getParameterName()  => new MethodQueryParameter('removeRuleTemplateFromLinks'),
        ], $this->editableQueryParameters);
    }
