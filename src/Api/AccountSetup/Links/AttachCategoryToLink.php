...
/**
 * @see https://docs.iovox.com/display/RA/attachCategoryToLink
 */
class AttachCategoryToLink extends AbstractLinks implements AttachCategoryToLinkInterface
{
    ...
    public function executeQuery(AttachCategoryToLinkPayload $payload): bool
    {
        ...
        $validate = $this->validator->validate($payload, null, [AttachCategoryToLinkPayload::GROUP_ATTACH]);
        if ($validate->count() > 0) {
            throw new ValidationPayloadException($validate);
        }

        $query->setContent($this->serializer->serialize($payload, 'xml', ['groups' => [AttachCategoryToLinkPayload::GROUP_ATTACH]]));
    ...
    protected function setQueryParameters(): void
    {
        ...
        $this->allQueryParameters = array_merge([
            ...
            MethodQueryParameter::getParameterName()  => new MethodQueryParameter('attachCategoryToLink'),
        ], $this->editableQueryParameters);
    }
