...
class RemoveCategoryFromLink extends AbstractLinks implements RemoveCategoryFromLinkInterface
{
    ...
    public function executeQuery(RemoveCategoryFromLinkPayload $payload): bool
    {
        ...
        $validate = $this->validator->validate($payload, null, [RemoveCategoryFromLinkPayload::GROUP_REMOVE]);
        if ($validate->count() > 0) {
            throw new ValidationPayloadException($validate);
        }

        $query->setContent($this->serializer->serialize($payload, 'xml', ['groups' => [RemoveCategoryFromLinkPayload::GROUP_REMOVE]]));
    ...
    protected function setQueryParameters(): void
    {
        ...
        $this->allQueryParameters = array_merge([
            ...
            MethodQueryParameter::getParameterName()  => new MethodQueryParameter('removeCategoryFromLink'),
        ], $this->editableQueryParameters);
    }
