...
class AttachVoxnumberToLink extends AbstractLinks
{
    ...
    public function executeQuery(AttachVoxnumberToLinkPayload $payload): bool
    {
        ...
        $validate = $this->validator->validate($payload, null, [AttachVoxnumberToLinkPayload::GROUP_ATTACH]);
        if ($validate->count() > 0) {
            throw new ValidationPayloadException($validate);
        }

        $query->setContent($this->serializer->serialize($payload, 'xml', ['groups' => [AttachVoxnumberToLinkPayload::GROUP_ATTACH]]));
    ...
    protected function setQueryParameters(): void
    {
        ...
        $this->allQueryParameters = array_merge([
            ...
            MethodQueryParameter::getParameterName()  => new MethodQueryParameter('attachVoxnumberToLink'),
        ], $this->editableQueryParameters);
    }
