...
/**
 * @see https://docs.iovox.com/display/RA/removeVoxnumberFromLink
 */
class RemoveVoxnumberFromLink extends AbstractLinks implements RemoveVoxnumberFromLinkInterface
{
    ...
    public function executeQuery(RemoveVoxnumberFromLinkPayload $payload): bool
    {
        ...
        $validate = $this->validator->validate($payload, null, [RemoveVoxnumberFromLinkPayload::GROUP_REMOVE]);
        if ($validate->count() > 0) {
            throw new ValidationPayloadException($validate);
        }

        $query->setContent($this->serializer->serialize($payload, 'xml', ['groups' => [RemoveVoxnumberFromLinkPayload::GROUP_REMOVE]]));
    ...
    protected function setQueryParameters(): void
    {
        ...
        $this->allQueryParameters = array_merge([
            ...
            MethodQueryParameter::getParameterName()  => new MethodQueryParameter('removeVoxnumberFromLink'),
        ], $this->editableQueryParameters);
    }
